...
export default function Home() {
  ...
  const connectWallet = async (
    walletType: "metamask" | "rabby" | "coinbase"
  ) => {
    ...
    if (eth.providers?.length) {
      if (walletType === "rabby") {
        ...
        provider = eth.providers.find(
          (p: any) =>
            p.isMetaMask && !p.isRabby && !p.isCoinbaseWallet
        );
      }
    } else {
      if (walletType === "rabby" && eth.isRabby) {
        provider = eth;
      } else if (walletType === "coinbase" && eth.isCoinbaseWallet) {
        provider = eth;
      } else if (
        walletType === "metamask" &&
        eth.isMetaMask &&
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const injectedProvider = activeProvider || (window as any).ethereum;
      ...
      // SEND TX
      const tx = await contract.consult();

      setTxHash(tx.hash);

      // SHOW QUOTE
      setQuote(
        quotes[getUniqueQuoteIndex(walletAddress, tx.hash)]
      );
          ...
          <div className="w-full max-w-sm bg-[#0a0a0c] border border-white/10 rounded-[32px] p-8 shadow-2xl relative">
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <div className="relative w-6 h-6">
                  {/* Inline SVG ensuring the logo always renders */}
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col text-[14px] md:text-[16px] text-white/90 font-mono tracking-[0.2em] gap-3 border-l-2 border-blue-600/50 pl-6 py-1">
              {[
                ...
                >
                  <span className="text-blue-600 text-[10px]">0{i + 1}</span>{" "}
                  {text}
                </span>
              ))}
            </div>
