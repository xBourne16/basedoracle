...
export default function Home() {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (tx) {
        const index = getUniqueQuoteIndex(walletAddress, tx);
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-center p-4 bg-[#020204] overflow-hidden selection:bg-blue-600/40">
      ...
      <nav className="fixed top-0 w-full p-8 flex justify-between items-start z-50">
        ...
        <button 
          ...
          className="group relative flex items-center gap-3 px-7 py-3 bg-white/[0.04] backdrop-blur-2xl border border-white/10 rounded-full transition-all duration-500 hover:border-blue-500/60 hover:bg-white/[0.1] active:scale-95 shadow-[0_0_20px_rgba(59,130,246,0.05)]"
        >
      ...
      <div className="absolute inset-0 z-0 opacity-[0.06] pointer-events-none">
        <Image src="/always_has_been.png" alt="BG" fill className="object-cover contrast-125" />
      </div>

      <div className={`absolute right-[8%] top-[15%] w-[600px] h-[550px] z-1 transition-all duration-1000 ${glowIntensity} pointer-events-none`}>
        <Image src="/crypto_scribble.png" alt="Oracle" fill className="object-contain grayscale brightness-125 contrast-110" />
      </div>

      {/* ANA KART */}
      <div className={`relative z-10 w-full max-w-6xl flex flex-col items-center transition-all lg:pr-32 ${isAnimating ? 'scale-95 blur-sm' : ''}`}>
        ...
        <div className="relative w-full max-w-2xl bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[50px] p-16 shadow-2xl md:-translate-x-12">
          ...
          <div className="mt-16 flex justify-end">
            <button 
              ...
              className="px-14 py-6 bg-white text-black font-black rounded-full hover:bg-blue-600 hover:text-white hover:scale-105 transition-all text-[10px] uppercase tracking-[0.3em]"
            >
      ...
      {/* FOOTER */}
      <footer className="fixed bottom-10 w-full px-12 flex justify-between items-end z-20">
        
        {/* SOL ALT: Weekend Protocol */}
        <div className="flex flex-col gap-4 group">
          <div className="flex flex-col gap-1">
            <span className="text-[13px] text-blue-500 font-black uppercase tracking-[0.4em]">
              Weekend Protocol:
            </span>
            <span className="text-[10px] text-white/30 uppercase tracking-[0.2em] italic font-medium">
              Current directives for the Base ecosystem
            </span>
          </div>
        ...
        {/* SAĞ ALT: Branding & X Entegrasyonu (X adresi kutunun altında) */}
        <div className="flex flex-col items-end gap-3">
          ...
          >
            <span className="text-[10px] text-white/40 font-mono tracking-[0.3em] group-hover:text-blue-400 transition-colors uppercase">
              @np0int
            </span>
