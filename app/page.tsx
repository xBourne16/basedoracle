...
export default function Home() {
  ...
  const [activeProvider, setActiveProvider] = useState<any>(null); // Provider'ı hafızada tutmak kritik
  ...
  // CÜZDAN BAĞLAMA: Rabby ve MetaMask'ı net bir şekilde ayırır
  const connectWallet = async (walletType: 'metamask' | 'rabby' | 'wc') => {
    if (typeof window === "undefined") return;
    
    if (walletType === 'wc') {
      alert("WalletConnect yakında eklenecek.");
      return;
    }
    ...
    // Eğer birden fazla provider varsa (MetaMask + Rabby gibi)
    ...
    try {
      ...
      setActiveProvider(provider); // Seçilen cüzdanı kaydet
  ...
  // İŞLEM GÖNDERME: TX Failed hatasını önlemek için optimize edildi
  const handleAction = async () => {
    ...
    try {
      ...
      const txParameters = {
        ...
        data: '0x62734346',
        value: '0x0',
        gas: '0x3D090', // 250,000 gas limiti (Base için güvenli aralık)
      };
      ...
      // Onay bekleme (Sözün hemen gelmesi için)
      let receipt = null;
      while (receipt === null) {
      ...
      alert("Transaction failed! Lütfen Base ağında olduğunuzdan emin olun.");
    } finally {
          ...
          <div className="w-full max-w-sm bg-[#0a0a0c] border border-white/10 rounded-[32px] p-8 shadow-2xl relative">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => connectWallet('rabby')} className="flex items-center justify-between px-6 py-5 bg-white/[0.03] border border-white/5 rounded-2xl hover:bg-white/10 transition-all group">
                ...
                <div className="relative w-7 h-7"><Image src="/rabby_logo.png" alt="Rabby" fill className="object-contain" /></div>
              </button>
        ...
        <div className="relative w-full max-w-2xl bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-[50px] p-16 shadow-2xl md:-translate-x-12">
          ...
          <div className="mt-16 flex justify-end relative z-[60]">
            <button 
              ...
              className="relative z-[70] px-14 py-6 bg-white text-black font-black rounded-full hover:bg-blue-600 hover:text-white hover:scale-105 active:scale-95 transition-all text-[10px] uppercase tracking-[0.3em] cursor-pointer disabled:opacity-50 shadow-xl"
            >
